...
- [x] Implement correct follow-up draft creation from an existing case. `Create Follow-up Draft` opens `/cases/new?followupOf=...`, loads the source case, and renders a new unsaved draft with source data cloned and persistence IDs stripped; covered by `followupDraft.test.ts`.
