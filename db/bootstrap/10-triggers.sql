...
-- Canonical audit append primitive shared by row triggers and explicit
-- domain-level audit events. The actor and compliance metadata come from the
-- transaction context, never from a caller-supplied user id.
...
-- Audit trigger for tables that use submission_id UUID as the primary key.
...
CREATE TRIGGER audit_submission_events AFTER INSERT OR UPDATE OR DELETE ON submission_events
    FOR EACH ROW EXECUTE FUNCTION audit_trigger_function();

CREATE TRIGGER audit_submission_dispatch_state AFTER INSERT OR UPDATE OR DELETE ON submission_dispatch_state
    FOR EACH ROW EXECUTE FUNCTION audit_trigger_function_with_submission_id();

CREATE TRIGGER audit_submission_idempotency AFTER INSERT OR UPDATE OR DELETE ON submission_idempotency
    FOR EACH ROW EXECUTE FUNCTION audit_trigger_function();
...
CREATE TRIGGER update_case_field_notations_updated_at
  BEFORE UPDATE ON case_field_notations
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER audit_case_field_notations
  AFTER INSERT OR UPDATE OR DELETE ON case_field_notations
  FOR EACH ROW EXECUTE FUNCTION audit_trigger_function();
...
ALTER TABLE case_field_notations ENABLE ROW LEVEL SECURITY;
ALTER TABLE case_field_notations FORCE ROW LEVEL SECURITY;
CREATE POLICY case_field_notations_via_case ON case_field_notations
  FOR ALL TO e2br3_app_role
  USING (EXISTS (
    SELECT 1 FROM cases c
    WHERE c.id = case_field_notations.case_id
      AND (c.organization_id = current_organization_id() OR is_current_user_admin())
  ))
  WITH CHECK (EXISTS (
    SELECT 1 FROM cases c
