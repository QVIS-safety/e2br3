...
async fn list_active_substances(
	...
	let sql = "SELECT * FROM drug_active_substances WHERE drug_id = ANY($1) AND deleted = false ORDER BY drug_id, sequence_number, id";
	mm.dbx()
...
async fn list_dosages(
	...
	let drug_ids: Vec<Uuid> = drugs.iter().map(|drug| drug.id).collect();
	let sql = "SELECT * FROM dosage_information WHERE drug_id = ANY($1) AND deleted = false ORDER BY drug_id, sequence_number, id";
	mm.dbx()
...
async fn list_indications(
	...
	let drug_ids: Vec<Uuid> = drugs.iter().map(|drug| drug.id).collect();
	let sql = "SELECT * FROM drug_indications WHERE drug_id = ANY($1) AND deleted = false ORDER BY drug_id, sequence_number, id";
	mm.dbx()
...
async fn list_relatedness_assessments(
	...
	let sql = "SELECT * FROM relatedness_assessments WHERE drug_reaction_assessment_id = ANY($1) AND deleted = false ORDER BY drug_reaction_assessment_id, sequence_number, id";
	mm.dbx()
		.fetch_all(
...
async fn list_patient_identifiers(
	...
	let sql = "SELECT * FROM patient_identifiers WHERE patient_id = $1 AND deleted = false ORDER BY sequence_number, id";
	mm.dbx()
...
async fn list_medical_history(
	...
	let sql = "SELECT * FROM medical_history_episodes WHERE patient_id = $1 AND deleted = false ORDER BY sequence_number, id";
	mm.dbx()
...
async fn list_past_drugs(
	...
	let sql =
		"SELECT * FROM past_drug_history WHERE patient_id = $1 AND deleted = false ORDER BY sequence_number, id";
	mm.dbx()
...
async fn list_reported_causes_of_death(
	...
	let sql = "SELECT * FROM reported_causes_of_death WHERE death_info_id = $1 AND deleted = false ORDER BY sequence_number, id";
	mm.dbx()
		.fetch_all(
...
async fn list_autopsy_causes_of_death(
	...
	let sql = "SELECT * FROM autopsy_causes_of_death WHERE death_info_id = $1 AND deleted = false ORDER BY sequence_number, id";
	mm.dbx()
	...
	let rows = mm
		...
		.fetch_all(
			sqlx::query_as::<_, ParentInformation>(
				"SELECT * FROM parent_information WHERE patient_id = $1 AND deleted = false ORDER BY created_at, id",
			)
			.bind(patient.id),
		)
		.await?;
	...
	let rows = mm
		...
		.fetch_all(
			sqlx::query_as::<_, ParentMedicalHistory>(
				"SELECT * FROM parent_medical_history WHERE parent_id = ANY($1) AND deleted = false ORDER BY parent_id, sequence_number, id",
			)
			.bind(parent_ids),
		)
		.await?;
	...
	let rows = mm
		...
		.fetch_all(
			sqlx::query_as::<_, ParentPastDrugHistory>(
				"SELECT * FROM parent_past_drug_history WHERE parent_id = ANY($1) AND deleted = false ORDER BY parent_id, sequence_number, id",
			)
			.bind(parent_ids),
		)
		.await?;
