		...
		let sql = format!(
			"UPDATE {}
			 SET test_name = CASE WHEN 'test_name' = ANY($17) THEN NULL ELSE COALESCE($2, test_name) END,
			     ...
			     test_result_qualifier = CASE WHEN 'test_result_qualifier' = ANY($17) OR 'test_result_value' = ANY($17) THEN NULL ELSE CASE WHEN $8 IS NOT NULL THEN $9 ELSE COALESCE($9, test_result_qualifier) END END,
		...
		let sql = format!(
			"UPDATE {}
			 SET test_name = CASE WHEN 'test_name' = ANY($18) THEN NULL ELSE COALESCE($3, test_name) END,
			     ...
			     test_result_qualifier = CASE WHEN 'test_result_qualifier' = ANY($18) OR 'test_result_value' = ANY($18) THEN NULL ELSE CASE WHEN $9 IS NOT NULL THEN $10 ELSE COALESCE($10, test_result_qualifier) END END,
