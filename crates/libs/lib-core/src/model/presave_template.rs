				...
				Some(value) => {
					Some(RegulatoryAuthority::parse(value).ok_or_else(|| {
						crate::model::Error::Store(format!(
							"invalid presave authority: {value}"
						))
					})?)
				}
				None => None,
...
pub struct PresaveTemplateListFilter {
	pub entity_type: Option<PresaveEntityType>,
	pub authority: Option<RegulatoryAuthority>,
...
impl PresaveTemplateBmc {
	pub async fn create(
		...
		let sql = "INSERT INTO presave_templates (organization_id, entity_type, authority, name, description, data, created_by, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW()) RETURNING id";
		let res = dbx
			.fetch_one(
				sqlx::query_as::<_, (Uuid,)>(sql)
					.bind(ctx.organization_id())
					.bind(template_c.entity_type.as_str())
					.bind(template_c.authority.map(RegulatoryAuthority::as_str))
	...
	pub async fn list(ctx: &Ctx, mm: &ModelManager) -> Result<Vec<PresaveTemplate>> {
		Self::list_filtered(ctx, mm, PresaveTemplateListFilter::default()).await
	}

	pub async fn list_filtered(
		ctx: &Ctx,
		mm: &ModelManager,
		filter: PresaveTemplateListFilter,
			...
			"SELECT * FROM {} \
			 WHERE organization_id = $4 \
			   ...
			   AND ( \
			     $2::text IS NULL \
			     OR authority = $2 \
			     OR ($3 = true AND authority IS NULL) \
			   ) \
			 ORDER BY (authority IS NULL) ASC, updated_at DESC \
			 LIMIT 1000",
			Self::TABLE
		);
		let rows = match dbx
		...
		let count = match dbx
			.execute(
				sqlx::query(&sql)
					...
					.bind(template_u.authority.map(RegulatoryAuthority::as_str))
