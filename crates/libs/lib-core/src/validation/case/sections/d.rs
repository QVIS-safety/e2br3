...
use crate::model::patient::{
	...
	PastDrugHistory, ReportedCauseOfDeath,
};
...
mod golden_companion_tests {
	...
	fn both_present_is_silent() {
		assert_eq!(medhist_codes(medhist(Some("10000001"), Some("27.0"))), Vec::new());
	}
	...
	fn reported_cause_codes(
		cause: ReportedCauseOfDeath,
	) -> Vec<(String, String)> {
		let mut ctx = empty_ctx();
	...
	fn reported_cause_present_without_comment_flags_or_trigger_rule() {
		...
		assert_eq!(
			reported_cause_codes(reported_cause(Some("10000001"), Some("27.0"), None)),
			vec![(
