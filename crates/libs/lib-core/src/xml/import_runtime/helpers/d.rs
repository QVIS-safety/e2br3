...
use crate::xml::import_runtime::shared::{
	build_initials, clamp_str, first_attr, first_text, first_text_root, first_value,
	first_value_root, initials_from_name_text, normalize_code, normalize_code3,
...
#[allow(dead_code)]
pub(crate) fn parse_patient_information(
	xml: &[u8],
) -> Result<Option<PatientImport>> {
	let xml_str = std::str::from_utf8(xml).map_err(|err| Error::InvalidXml {
		message: format!("XML not valid UTF-8: {err}"),
		line: None,
		column: None,
