...
#[test]
#[serial]
fn reserved_email_rows_grant_nothing() {
	let _registry = RegistryGuard::new();
	assert!(profile("home_email", true, false, false, false).is_empty());
	assert!(profile("home_email", false, true, false, false).is_empty());
	assert!(profile("email_report_due", true, true, true, true).is_empty());
}
