...
async fn save_f_r_create() -> Result<()> {
	...
	assert_eq!(row.test_date, None);
	assert_eq!(row.test_date_null_flavor, None);
	assert_eq!(row.test_name, "ALT");
	assert_eq!(row.test_meddra_version, None);
	assert_eq!(row.test_meddra_code, None);
	assert_eq!(row.test_result_code, None);
	assert_eq!(row.test_result_value, None);
	assert_eq!(row.test_result_unit, None);
	assert_eq!(row.result_unstructured, None);
	assert_eq!(row.normal_low_value, None);
	assert_eq!(row.normal_high_value, None);
	assert_eq!(row.comments, None);
	assert_eq!(row.more_info_available, None);
	finish(&mm).await
}
