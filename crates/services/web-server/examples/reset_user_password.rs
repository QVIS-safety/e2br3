...
async fn main() -> Result<()> {
	...
	let password = std::env::var("E2BR3_RESET_PASSWORD").map_err(|_| {
		"E2BR3_RESET_PASSWORD is required".to_string()
	})?;
	...
	UserBmc::update_pwd_and_clear_must_change(&ctx, &mm, user.id, &password)
		.await?;
