...
drug_single_field_test!(
	...
	|value| { assert_f64(value, "cumulative_dose_first_reaction_value", 150.0); }
);
drug_single_field_test!(
	...
	|value| { assert_str(value, "cumulative_dose_first_reaction_unit", "mg"); }
);
drug_single_field_test!(
	...
	|value| { assert_f64(value, "gestation_period_exposure_value", 10.0); }
);
drug_single_field_test!(
	save_g_k_gestation_period_exposure_unit_only,
	"G.k.gestation_period_exposure_unit",
	json!({"gestation_period_exposure_unit": "wk"}),
	|value| { assert_str(value, "gestation_period_exposure_unit", "wk"); }
);
drug_single_field_test!(
	save_g_k_dosage_text_only,
	"G.k.dosage_text",
	json!({"dosage_text": "Dosage"}),
	|value| { assert_str(value, "dosage_text", "Dosage"); }
);
drug_single_field_test!(
	save_g_k_action_taken_only,
	"G.k.action_taken",
	json!({"action_taken": "1"}),
	|value| { assert_str(value, "action_taken", "1"); }
);
drug_single_field_test!(
	save_g_k_rechallenge_only,
	"G.k.rechallenge",
	json!({"rechallenge": "2"}),
	|value| { assert_str(value, "rechallenge", "2"); }
);
drug_single_field_test!(
...
drug_single_field_test!(
	...
	|value: &serde_json::Value| {
		assert_eq!(value["data"]["drug_additional_info_codes_json"], json!(["A", "B"]));
	}
);
drug_single_field_test!(
	...
	|value| { assert_str(value, "fda_specialized_product_category", "device"); }
);
drug_single_field_test!(
...
dosage_single_field_test!(
	...
	|value| { assert_str(value, "first_administration_date_null_flavor", "NI"); }
);
dosage_single_field_test!(
	save_g_k_4_r_last_administration_date_only,
	"G.k.4.r.last_administration_date",
	json!({"last_administration_date": [2024, 2, 2]}),
	|value| { assert_date_tuple(value, "last_administration_date", &[2024, 33]); }
);
dosage_single_field_test!(
...
indication_single_field_test!(
	...
	|value| { assert_str(value, "indication_meddra_version", "28.0"); }
);
indication_single_field_test!(
	save_g_k_6_r_indication_meddra_code_only,
	"G.k.6.r.indication_meddra_code",
	json!({"indication_meddra_code": "901"}),
	|value| { assert_str(value, "indication_meddra_code", "901"); }
);
...
assessment_single_field_test!(
	...
	|value| { assert_f64(value, "administration_start_interval_value", 2.0); }
);
assessment_single_field_test!(
