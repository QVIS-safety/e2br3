use crate::persist_workflow::{
	...
	fill_section_c, force_case_validated_for_export, save_case, setup,
	validate_case_fda,
};
...
#[serial]
#[tokio::test]
async fn c_section_forms_persist_to_exported_xml() -> crate::common::Result<()> {
	...
	assert!(xml.contains("Sender Persist Org"));
	assert!(xml.contains("PERSIST-REG-1"));
	Ok(())
}
