...
async fn setup_imported_case() -> Result<(axum::Router, String, String)> {
	...
	if std::env::var("E2BR3_XSD_PATH").is_err() {
		return Err("E2BR3_XSD_PATH not set".into());
	}
...
async fn test_roundtrip_dg_remaining_14_fields() -> Result<()> {
	...
	let sentinel_parent_route = format!("RTDG33-{}", Uuid::new_v4().simple());
	let sentinel_dose_form = format!("RTDG34-{}", Uuid::new_v4().simple());
	...
	let (status, body) = request_json(
		...
		Some(serde_json::json!({
			"data": {
				...
				"parent_route": sentinel_parent_route,
				"dose_form": sentinel_dose_form
			}
		})),
	)
	.await?;
	...
	for expected in [
		...
		sentinel_parent_route.as_str(),
		sentinel_dose_form.as_str(),
	] {
		assert!(
			xml.contains(expected),
