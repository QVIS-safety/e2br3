...
#[serial]
#[tokio::test]
async fn test_sender_presave_routing_profile_excludes_deleted_gateway_options(
) -> Result<()> {
	let mm = init_test_mm().await?;
	let seed = seed_org_with_users(&mm, "adminpwd", "viewpwd").await?;
	let admin_token = generate_web_token(&seed.admin.email, seed.admin.token_salt)?;
	...
	let deleted_sender_id = create_sender_presave(
		&app,
		&admin_cookie,
		&format!("Deleted Gateway Sender {}", Uuid::new_v4()),
		"SEND-DELETED-GATEWAY",
	)
	.await?;
	let (status, value) = request_json(
		&app,
		"GET",
		&admin_cookie,
		format!("/api/presaves/senders/{deleted_sender_id}/details"),
		None,
	)
	.await?;
	assert_eq!(status, StatusCode::OK, "{value:?}");
	let gateway_id = value["data"]["gateways"]
		.as_array()
		.and_then(|rows| rows.first())
		.and_then(|row| row["id"].as_str())
		.ok_or("missing gateway id")?;
	let (status, value) = request_json(
		&app,
		"PATCH",
		&admin_cookie,
		format!("/api/presaves/senders/{deleted_sender_id}/gateways/{gateway_id}"),
	...
	create_sender_presave(
		&app,
		&admin_cookie,
		&format!("Active Gateway Sender {}", Uuid::new_v4()),
		"SEND-ACTIVE-GATEWAY",
	)
	.await?;
	let (status, profile) = request_json(
	...
	.await?;
	assert_eq!(status, StatusCode::OK, "{profile:?}");
	let senders = profile["data"]["availableSenders"]
		.as_array()
		.ok_or("missing available senders")?;
	assert!(
		senders
			.iter()
			.any(|row| row["senderIdentifier"] == "SEND-ACTIVE-GATEWAY"),
		"{profile:?}"
	);
	assert!(
		!senders
			.iter()
			.any(|row| row["senderIdentifier"] == "SEND-DELETED-GATEWAY"),
		"{profile:?}"
	);
	Ok(())
}
